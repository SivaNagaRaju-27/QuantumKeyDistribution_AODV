...
#import enc_message,dec_message
import algo
...
class aodv(threading.Thread):
    ...
    # Constructor
    def __init__(self):
        print("Entered into the aodv.py")
    ...
    def aodv_process_user_message(self, message):
        ...
        if (receiver == self.node_id):
            ...
            print("Message is forwarded to: ",next_hop)
    ...
    def aodv_send_message(self, from_tester):
        ...
        user = input("Do you want to encrypt [y/n] ?")
        e_msg = ""
        '''qkd = BB84(bit_length)
        qkd.run()
        sender_key = qkd.sender_key()
        sender_key = bytes(sender_key)'''
        if (user=='y'):
            e_msg=algo.encrypt_msg(message_data)
        #message = message_type + ":" + source + ":" + dest + ":" + message_data
        message = message_type + ":" + source + ":" + dest + ":" + e_msg
        ...
        if dest in self.routing_table.keys():
            ...
            if (destination['Status'] == 'Inactive'):
                ...
                print("Message is forwarded to next_hop",next_hop)
                print("Message is ",message_data)
    ...
    def aodv_show_routing_table(self, from_tester):
        ...
        for r in self.routing_table.values():
            print(str(r['Destination']) + "              " + str(r['Next-Hop']) + "           " + str(r['Seq-No']) + "          " + str(r['Hop-Count']) + "             " + str(r['Status']))
        print("")
    ...
    def aodv_show_messages(self, from_tester):
        ...
        for m in self.message_box.values():
            ...
            user=input("Do you want to decrypt [y/n] ? ")
            if user=='y':
                d_msg = algo.decrypt_msg(m["Message"])
                print("The message is : ",d_msg)
        print("")
